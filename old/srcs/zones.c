...
t_zone *create_zone(zone_type_t type, size_t size) {
	...
	if (type == ZONE_TINY) {
		...
		zone_size =
		  (TINY_MAX_SIZE + sizeof(t_block)) * MIN_ALLOC_PER_ZONE + sizeof(t_zone);
	} else if (type == ZONE_SMALL) {
		// Size for at least 100 SMALL allocations + zone metadata + block headers
		zone_size =
		  (SMALL_MAX_SIZE + sizeof(t_block)) * MIN_ALLOC_PER_ZONE + sizeof(t_zone);
	} else { // ZONE_LARGE
	...
	void *addr = mmap(NULL, zone_size, PROT_READ | PROT_WRITE,
