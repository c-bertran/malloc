...
/**
 * @brief Get aligned user data pointer
 * @param block Memory block
 * @return Aligned pointer to user data area
 */
void *get_aligned_user_ptr(t_block *block);
 ...
 * @note This function is only active when DEBUG_MALLOC is enabled
 */
void log_operation(const char *operation, void *ptr, size_t size);

/**
 * @brief Print number as a string to a file descriptor
 * @param n Number to print
 * @param base Base for conversion (e.g., 10 for decimal)
 * @param charset Character set for digits (e.g., "0123456789ABCDEF")
